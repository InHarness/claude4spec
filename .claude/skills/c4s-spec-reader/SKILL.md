...
command below carries its identity (`--project 'app-spec' --workspace 'claude4spec'`), so it works from any cwd,
...
XML tag names 1:1 — append `--project 'app-spec' --workspace 'claude4spec'` to every command below.

| XML tag | CLI equivalent |
|---------|----------------|
| `<inline_mention type="endpoint" slug="X"/>` | `c4s inline_mention --type endpoint --slug X --project 'app-spec' --workspace 'claude4spec'` |
| `<single_element type="dto" slug="X"/>` | `c4s single_element --type dto --slug X --project 'app-spec' --workspace 'claude4spec'` |
| `<element_list type="endpoint" slugs="a,b,c"/>` | `c4s element_list --type endpoint --slugs a,b,c --project 'app-spec' --workspace 'claude4spec'` |
| `<tagged_list type="dto" tags="auth" filter="and"/>` | `c4s tagged_list --type dto --tags auth --filter and --project 'app-spec' --workspace 'claude4spec'` |
| `<tagged_list_mixed tags="public"/>` | `c4s tagged_list_mixed --tags public --project 'app-spec' --workspace 'claude4spec'` |

## Expanding a whole page

```sh
c4s resolve some-page.md --project 'app-spec' --workspace 'claude4spec'                # writes markdown with tags expanded inline
c4s resolve some-page.md --format json --project 'app-spec' --workspace 'claude4spec'   # writes { content, resolved: [...] }
```
...
```sh
c4s search-pages --query "<phrase>" --project 'app-spec' --workspace 'claude4spec'   # hits carry an anchor (indexed root)
c4s get-sections --anchors a,b,c --project 'app-spec' --workspace 'claude4spec'      # bodies of several sections in ONE call
```
...
```sh
c4s get-page-outline --root-id pages --path some/page.md --project 'app-spec' --workspace 'claude4spec'
c4s list-pages --root-id pages [--prefix modules/] --project 'app-spec' --workspace 'claude4spec'
```

Whole page, when you really want the file:

```sh
c4s get-page --root-id pages --path some/page.md --project 'app-spec' --workspace 'claude4spec'
```
...
- `c4s catalog --project 'app-spec' --workspace 'claude4spec'` — the ENTRY POINT: page roots with their properties, active entity types with counts + version + description + roleNoun + mcpToolsLine per type, tag count. Start here; it is cheap and it tells you what else is worth asking.
- `c4s describe --type <t> --project 'app-spec' --workspace 'claude4spec'` — what a type IS: JSON Schemas, the value `constraints` a write must satisfy, `selectableFields` (the names `--select` accepts), `contentFields` (fields no generic read carries, each with the operation that issues them) and `searchableFields`. Worth calling before a READ, not just before a write.
- `c4s list-tags [--with-counts] [--min-count <n>] [--co-occurring-with <slug>] --project 'app-spec' --workspace 'claude4spec'` — the project tags. Counts are OFF by default (they are a product of tags by types); `--co-occurring-with` returns the tags sharing entities with one you name, which is how you discover a taxonomy without already knowing it.
- `c4s list-entities --type endpoint [--tags auth] [--tag-filter and|or] [--sort createdAt|title|slug] [--dir asc|desc] [--mode items|count] --project 'app-spec' --workspace 'claude4spec'` — full paginated traversal of one type. Rows are `{ slug, title }`: discovery answers with keys, and you follow up with `get-entities` for content. Only the default `createdAt` order has a write-stable offset window.
- `c4s get-entities --type dto --slugs a,b,c [--select <f1,f2>] --project 'app-spec' --workspace 'claude4spec'` — several entities in one call. `--select` names top-level fields (`slug`, `title` and `tags` always come back); omit it for every field except content-bearing ones, or pass `--select=` for the identity skeleton alone.
- `c4s get-field-content --type diagram --slug <s> --field source --project 'app-spec' --workspace 'claude4spec'` — the content of ONE content-bearing field. Such a field is carried by no generic read (you get `has<Field>` / `<field>Bytes` instead), so this is the only way to read one.
- `c4s search-entities --type ac --query "<phrase>" [--fields <f1,f2>] --project 'app-spec' --workspace 'claude4spec'` — text search inside one type (the type is required); the output always declares `searchedFields`, so an empty result is distinguishable from an unsearched field.
- `c4s resolve-identity --query "<fragment>" [--types endpoint,dto] --project 'app-spec' --workspace 'claude4spec'` — the only cross-type command: "what is this called?" from a fragment of a name.
- `c4s check-consistency [--severity <s>] [--rule <r>] --project 'app-spec' --workspace 'claude4spec'` — the spec's own diagnostics (broken references, drift between disk and index).
...
```sh
c4s ask "<question>" --project 'app-spec' --workspace 'claude4spec'
```
...
`AMBIGUOUS_PROJECT`, this skill's baked-in `--project 'app-spec' --workspace 'claude4spec'` identity no longer
