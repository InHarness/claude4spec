...
2a. **A layer file does not name modules.** In `layers/LY-<slug>.md` the only place a module identifier may appear is the `Implementor module:` slot. Rule 2's layer-purity test already asks whether the content would survive deleting a module; this is that same rule restated so it can be settled on the file's text alone, because a rule without a named symptom comes back from review as an opinion instead of as a deviation. Three symptoms, and each is decidable without judging the subject matter:

   - **No token matching `[Mm]\d+` in the file, outside the slot.** *Symptom:* `M03` or `m19` in the prose, a heading or a table of `layers/L1-db.md`.
   - **No `section_ref` anywhere in the file** — unconditionally, not only when it points at a module's page. *Symptom:* a `section_ref` construct in a layer file.
   - **The slot is not paraphrased beside itself.** It carries the one fact in the file that is about the *layer* rather than about a module — who implements it — which is exactly why a sentence repeating it ("the implementor is M07, see its L1 section") gives one fact two homes, which rule 2 forbids. *Symptom:* prose naming the implementor from its number and pointing at its section.

   *Two things are out of scope, and both get read wrong.* The `MNN`/`mNN` placeholder inside the embed pattern in `## Module slice schema` is a placeholder of the pattern, not a reference — it names no module. And `modules/MXX-<slug>/LY-<slug>.md` is a module slice, governed by the module's rules (§6 rule 5): the two files share a name and differ only by their directory, so the check reads the **path**, never the filename.

   When a violation is real, place the content by rule 2's filling-vs-behavior split: a rule about *filling* a module's section is absorbed into `## Module slice schema` as a field requirement; a description of framework *runtime* behavior moves to the implementor module's own section for this layer. Leave no breadcrumb behind — rule 9.
