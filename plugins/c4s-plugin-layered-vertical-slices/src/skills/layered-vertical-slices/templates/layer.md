...
Its prose also does not name modules: the `Implementor module:` slot below is the only
place a module identifier appears in this file, and the `MNN`/`mNN` inside the embed
pattern is a placeholder of the template, not a reference (SKILL.md §6 rule 2a).
