 ...
 *
 * `src/index.ts` is what the host's Node loader imports. One import edge from
 * there into a `.tsx` file put `react`, `react/jsx-runtime` and `lucide-react`
 * on the server's plugin-load path — and it was an innocuous-looking one:
 * `capabilities/commands.ts` importing two popover-kind STRING constants from
 * the popovers that use them, so a rename could not unhook the command from its
 * handler. Rollup faithfully hoisted the whole component module into a shared
 * chunk that the backend bundle imports on line 1.
 *
 ...
 * `PLUGIN_IMPORT_FAILED`, and a failed envelope means `endpoint` and `dto`
 * silently do not exist — the same total, quiet absence that took the first
 * smoke test of this release to notice.
 *
