...
import { endpointSlug } from '../../../identity.js';
import type { ChangedBy } from '../../../host-kit/host-types.js';
import { DomainError } from '../../../host-kit/errors.js';
import type { TagsServiceLike as TagsService } from '../../../host-kit/host-types.js';
...
} from '../../../host-kit/entity-crud-service.js';
...
export class EndpointService extends BaseEntityCrudService<Endpoint> {
  ...
  updateRaw(slug: string, input: EndpointUpdateInput, actor: ChangedBy, opts: MutateOpts = {}): Endpoint {
    const tx = this.db.transaction(() => {
      ...
      const stamp = resolveStampForUpdate('endpoint', opts, this.store, slug, current);
      this.db
        .prepare(
          `UPDATE endpoint
