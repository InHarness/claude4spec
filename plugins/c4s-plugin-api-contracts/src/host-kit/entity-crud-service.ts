/**
 * VENDORED from the host (`server/core/plugin-host/entity-crud-service.ts`).
 *
 * `BaseEntityCrudService` is deliberately NOT part of the published Host API —
 * the host describes it as "an in-process convenience for host-owned entities,
 * not a contract plugin authors depend on". This package is no longer a
 * host-owned entity, so it carries its own copy rather than reaching back into
 * the host for one. That is the point: an envelope must compile with nothing
 * but `@c4s/plugin-runtime`.
 *
 * The alternative — the external-plugin pattern of a hand-written crud-adapter
 * per type — would have rewritten both services, and rewriting is exactly what
 * the L9 byte-identity criterion forbids in this change.
 *
* Original docblock — M13: the write-side contract every entity plugin's L2 service satisfies so
