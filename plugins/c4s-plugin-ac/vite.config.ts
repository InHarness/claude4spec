 ...
 * error, and the host simply has no `ac`: no sidebar tab, no `/acs` routes, no
 * `ac-tools` MCP server and no `ac-audit` subagent. That is exactly how the
 * first smoke test of the api-contracts envelope failed, which is why this
 * `outDir` is copied verbatim rather than re-derived.
 *
 * Extraction is unaffected: the source tree is untouched, and a standalone repo
 * would point `outDir` back at its own `dist/`.
 */
/**
 * `@inharness-ai/agent-adapters` is external here and on no other envelope.
 *
 * This is the one envelope that runs an LLM turn of its own — the AC semantic
 * audit — and the adapter is a host dependency with a live process behind it,
 * not a library to vendor a second copy of. It resolves the same way `express`
 * and `better-sqlite3` do: up into the host's own `node_modules`. The turn's
 * SCOPE does not come with it — that arrives through `MountContext.agentScope`,
 * because the resolvers behind it read project config the host owns.
 */
const EXTERNAL = [
  '@c4s/plugin-runtime',
...
export default defineConfig({
  ...
  build: {
    ...
    sourcemap: true,
    target: 'es2022',
    outDir: path.resolve(import.meta.dirname, '../../dist/plugins/c4s-plugin-ac'),
  ...
  plugins: [
    {
       ...
       */
      closeBundle() {
        const out = path.resolve(import.meta.dirname, '../../dist/plugins/c4s-plugin-ac');
