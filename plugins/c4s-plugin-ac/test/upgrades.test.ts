...
import { acPayloadV1ToV2, acPayloadV2ToV3, acPayloadUpgrades } from '../src/entity/ac/upgrades.js';
import { acEntity } from '../src/entity/ac/index.js';
import {
  upgradePayload,
  PayloadUpgradeError,
} from '../../../src/server/serialization/payload-upgrade.js';
import type { SnapshotData } from '../../../src/server/serialization/types.js';
...
describe('acPayloadV2ToV3 — text and description collapse into title', () => {
  ...
  describe('the refusal', () => {
    ...
    it('surfaces as a PayloadUpgradeError from the chain, which the indexer skips on', () => {
      expect(() =>
        upgradePayload(acEntity, v2({ text: 'x'.repeat(600) }), 2),
...
describe('the ac chain as registered', () => {
  it('declares one step per version transition, ending at the manifest version', () => {
    ...
    expect(acPayloadUpgrades).toHaveLength(acEntity.payloadVersion - 1);
  });

  it('takes a v1 file all the way to v3 in one pass', () => {
    const v1 = { slug: 'ac-old', text: 'the rule holds', kind: 'requirement', status: 'active' };
    const { data, upgraded } = upgradePayload(acEntity, v1 as SnapshotData, 1);
