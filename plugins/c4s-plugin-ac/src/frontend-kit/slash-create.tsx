...
/**
 * The invocation context a form receives.
 *
 * `currentPath` joined `coords` on the event in 0.2.80. It is the page the
 * command was typed on, and it is a HINT in exactly the same sense: a form uses
 * it to default a field from context (this envelope's `/ac` derives the initial
 * tags from it), and a host that sends none, or a caret outside a page, leaves
 * it `null` rather than breaking the command.
 */
...
export function subscribeToSlashCreate(
  kind: string,
  onInvoke: (editor: EmbedEditor, ctx: SlashInvocation) => void,
): () => void {
  ...
  const handler = (ev: Event) => {
    ...
    onInvoke(detail.editor, {
      ...
      currentPath: detail.currentPath ?? null,
    });
  };
...
export function mountSlashCreatePopover(
  ...
  const unsubscribe = subscribeToSlashCreate(kind, (editor, { coords, currentPath }) => {
    void (async () => {
      ...
      root!.render(
        createElement(
          ...
          createElement(Component as never, { editor, coords, currentPath, onClose: close }),
        ),
      );
    })();
  });
