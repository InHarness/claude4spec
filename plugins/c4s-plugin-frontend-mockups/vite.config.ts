 ...
 * error, and the host simply has no `ui-view` and no `design-system`. That is
 * exactly how the first smoke test of the api-contracts envelope failed, which
 * is why this `outDir` is copied verbatim rather than re-derived.
 *
...
export default defineConfig({
  ...
  build: {
    ...
    sourcemap: true,
    target: 'es2022',
    outDir: path.resolve(import.meta.dirname, '../../dist/plugins/c4s-plugin-frontend-mockups'),
  ...
  plugins: [
    {
       ...
       */
      closeBundle() {
        const out = path.resolve(import.meta.dirname, '../../dist/plugins/c4s-plugin-frontend-mockups');
