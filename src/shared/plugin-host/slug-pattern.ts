/**
 * Host API 2.0.0 — `slugPattern`, the DATA that replaces `slugFrom`, the function.
 *
 ...
 *
 * The grammar is CLOSED: `literal | slugify(field) | truncate(n) | nanoid(n)`,
...
/**
 * A pattern, or an ordered chain of fallback patterns.
 *
 * The chain exists for `diagram`, whose slug is `slugify(caption)` →
 * `slugify(firstSourceIdentifier)` → `nanoid(8)`: the first alternative
...
/** Random-suffix source, injected so tests are deterministic and `shared/` stays dep-free. */
  ...
  for (const step of steps) {
    switch (step.op) {
      ...
      case 'slugify': {
         ...
         * `diagram` would never reach its `nanoid`, and an `ac` with no text
         * would be named `ac-x-0`.
         *
 ...
 */
export function evaluateSlugPattern(
  pattern: SlugPattern,
  data: Record<string, unknown>,
  nanoid: NanoidFn,
): string {
  for (const alternative of alternativesOf(pattern)) {
    const candidate = evalAlternative(alternative, data, nanoid);
...
/**
 * Evaluate a pattern for DISPLAY, without a random source.
 *
 * The client shows a prospective slug while the user types; the server computes
 * the real one, resolves collisions and is the only authority. Giving the client
 * a real `nanoid` would produce a preview that is guaranteed WRONG for any
 * pattern using one — a different random string every keystroke, none of them
 * the value that gets stored. A stable placeholder says "the host fills this in"
 * instead of lying with precision.
 *
...
export function previewSlugPattern(pattern: SlugPattern, data: Record<string, unknown>): string {
  return evaluateSlugPattern(pattern, data, (n) => '#'.repeat(n));
}
...
export function slugPatternFields(pattern: SlugPattern): string[] {
  ...
  for (const alternative of alternativesOf(pattern)) {
    for (const step of alternative) {
      if (step.op === 'slugify') fields.push(step.field);
    }
  }
  return fields;
}
...
export function slugPatternIsTotal(pattern: SlugPattern): boolean {
  return alternativesOf(pattern).some((alt) =>
    alt.some((s) => s.op === 'nanoid' || (s.op === 'literal' && s.value !== '')),
  );
}
