...
export function assertSerializationContribution(
  ...
  /**
   * 0.2.31 — the `diff` slot is REJECTED, not ignored.
   *
   * The semantic delta is generated by the host from the type's logical schema
   * and its `collection.identity` declarations; there is no slot left for a type
   * to contribute an algorithm to. Ignoring a stale `diff` would be the worst of
   * the three options: the author's function would sit there looking honoured
   * while every delta came from somewhere else. The envelope has no slot for it,
   * so an unknown key does not pass quietly.
   */
  if ('diff' in module && module.diff !== undefined) {
    throw new PluginManifestError(
      `entity "${type}" — the \`diff\` slot was removed in 0.2.31; the host generates the ` +
        'semantic delta from `data.schema` and the `collection: { kind, identity }` declarations. ' +
        'Declare element identity on the collections that have one instead',
    );
  }
 ...
 *
 * A package still shipping one is not merely out of date: its `payloadUpgrades`
 * sit one level too deep, so the host would find none. `payloadVersion` would
 * appear to be 1 with no steps declared, and every payload the plugin has ever
 * written would be read at the wrong shape without a word.
 *
