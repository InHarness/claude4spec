...
/**
 * 0.2.79 — the five plugin-surface operations' DTOs moved to `shared/discovery/`
 * so the published Host API mirror can state them without dragging this module's
 * host imports along. Re-exported here so every existing import site is unchanged.
 */
import type {
  DescribeTypesInput,
  DescribedType,
