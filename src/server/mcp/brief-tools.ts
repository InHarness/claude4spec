...
export function buildBriefToolsServer(ctx: BriefToolsContext): McpServerInstance {
  ...
  const updateBrief = mcpTool(
    ...
    [
      ...
      'You CANNOT modify frontmatter (type, from_release, to_release, generated_at,',
      'generator_version, implemented). `implemented` is owned by the implementer-agent',
      'in the target repo and toggled via filesystem edit, not via this MCP.',
