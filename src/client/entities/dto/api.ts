...
export const dtosApi = {
  async list(query: DtoListQuery = {}): Promise<Dto[]> {
    ...
    const data = await handle<{ dtos: Dto[] }>(await fetch(`/api/dtos${q}`));
    return data.dtos;
  },

  async get(slug: string): Promise<Dto> {
    return handle<Dto>(await fetch(`/api/dtos/${encodeURIComponent(slug)}`));
  },

  async create(input: DtoCreateInput): Promise<Dto> {
    return handle<Dto>(
      await fetch('/api/dtos', {
        method: 'POST',
  ...
  async update(slug: string, input: DtoUpdateInput): Promise<Dto> {
    return handle<Dto>(
      await fetch(`/api/dtos/${encodeURIComponent(slug)}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(input),
      })
  ...
  async remove(slug: string): Promise<{ deleted: true }> {
    return handle<{ deleted: true }>(
      await fetch(`/api/dtos/${encodeURIComponent(slug)}`, { method: 'DELETE' })
    );
  },
};
