...
- **BREAKING (agent): the Claude Code preset is off by default.** A missing `agent.claudeUsePreset` in `.claude4spec/config.json` now means `false` (it meant `true`): the chat agent receives only the prompt the project builds, as a raw system prompt, instead of having it appended to the built-in `claude_code` preset. There is no `$schemaVersion` bump and no migration, so this reaches every existing project that never set the field, and — because the field is read per turn — every conversation already under way, from its next turn. The server still wins over the request body, on `POST /api/chat` and `POST /api/threads/:id/ask` alike; `chat_thread.initial_system_prompt` and the `@inharness-ai/agent-adapters` default are unchanged. To restore the previous behaviour set `"agent": { "claudeUsePreset": true }` in `.claude4spec/config.json`, or tick "Append the Claude Code preset to the system prompt" in Settings › Agent, which now carries a note about the change.

- **Release descriptions are capped at 500 characters.** `createRelease` / `updateRelease` (REST `POST /api/releases`, `PATCH /api/releases/:nameOrId`, MCP `release_create` / `release_update`) refuse a description longer than 500 Unicode code points after trim with the new `400 RELEASE_DESCRIPTION_TOO_LONG` — never truncating it. An empty description on update is `400 RELEASE_DESCRIPTION_REQUIRED`, as on create. Only writes are checked: a longer description saved before this version is still read, rebuilt from `releasesDir` and restored untouched, and a rename-only update of it still passes. The create-release dialog shows a character counter.

- **BREAKING (CLI): `c4s read-brief` → `c4s get-brief`, `c4s file-patch` → `c4s create-patch`; operation `file_patch` → `create_patch`.** The CLI verbs now match the operations they render (`get_brief`, `create_patch`). Flags, defaults (`--kind drift`), exit codes and payloads are unchanged, and there is no alias: the old names answer `UNKNOWN_COMMAND`. The rename also reaches the M23 catalog row, the `POST /api/patches` description, the operation name in the `rest` channel audit, and the `patch-tools` MCP tool (`mcp__patch-tools__create_patch`). The MCP tool `get_brief` is unchanged. `c4s create-patch` now refuses a `--brief` that escapes `briefsDir` (`../`, absolute) locally with `INVALID_ARGS` (exit 4), before any request, the same guard `get-brief` has. `BRIEF_NOT_FOUND` and its hint still come from the server. The generated `c4s-brief-implementer` / `c4s-refactor` skills use the new verbs, and the implementer skill states that a failed `c4s` command stops the work rather than falling back to the specification's files. **Re-run `c4s install-skills`**, or an installed skill will call commands that no longer exist.

- **BREAKING (`get_sections`, fourth break): `includeSubtree` widens the SET of items, not an item's body.** One item PER SECTION: the requested anchors in input order and, with `includeSubtree: true`, every section beneath each of them as its own full item spliced in directly behind it in document order — indistinguishable from a requested one. A parent carries ONLY its own body (up to its first child heading) at either setting of the flag, and its `line_end` is the end of that own body; the indexed range and `content_hash` still run over the subtree, because the write side lives off them. The `{ anchor, coveredBy }` item variant is gone with the body it pointed at, and so is the `truncated` it inherited: an anchor that is both requested and inside another's subtree appears once, at its own input position. A third valve joins the anchors cap and the response budget: after expansion, more than `MAX_SECTION_ITEMS_PER_RESPONSE` (50) items is cut (error items count): every requested anchor keeps its item and the expansion is cut to a prefix in output order, so a large subtree can never push a later requested anchor out of the response with no error item to say so — with `truncated: true` and a `message` that names the ceiling and sends the caller to `get_page_outline` — never "retry with fewer anchors", which would return the same thing. The ceiling bites before the budget; the first item survives both. `edges` on a truncated item are parsed from the FULL OWN body, with no tags from child sections (the `section_entity_link` augmentation subtracts descendants' links). `get_page_outline.size` now measures the same own body, so a measurement before fetching has no hole for the expanded case. Both MCP servers paste one shared `GET_SECTIONS_RETURN` sentence set (thresholds interpolated from the core's constants), declare `includeSubtree` with `default: false`, and name `get_page_outline` in the ceiling hint. `c4s get-sections --include-subtree` renders the same flat list; no flag or exit code changes. An explicit anchor met inside another's expansion is skipped together with its subtree, so its descendants come back behind its own item. A page the index knows but the disk no longer has is a per-item `PAGE_NOT_FOUND` on every section of that page, not a failed batch. The own-body end is `ownEndOf`, the write side's definition, computed once per page; the `spec-explore` subagent prompt and `update_sections`' `content` description teach the new shape (a parent's body is not its whole range — `replace` still splices the subtree).
- **The `layered-vertical-slices` package documents decisions, not contracts — and each rule once.** The style's always-on core (`SKILL.md`, re-sent every turn until the first compaction) drops from 5 540 to under 1 000 words: role, concepts, layout, routing, package map. The quality rules and the two cross-cutting reading protocols move to `parts/` — source files outside the `files` map — and are spliced at import into the workflows whose steps need them (`daily` carries the whole catalogue, `bootstrap` placement + authoring, `patch` placement + the protocols, `brief` only the module-path pattern it can use): one home in source, N deliveries, no extra `load_skill_file`. Rule numbers (1–9, 2a, 3a) stay stable addresses; every decidable rule carries a `*Symptom:*` marker, and the check list is extracted from those markers at load and projected into `daily` step 5 and into the `spec-review` prompt, which imports the projection instead of telling the reviewer to re-derive it. Per thread the words in the window fall by 73 % (`brief`), 41 % (`patch`), 35 % (`bootstrap`) and 17 % (`daily`). `version: 1 → 2`.

  What left the package rather than moved: every restatement of a contract another source already publishes — `search_pages`' default page size and `get_sections`' anchor ceiling as literals, a nine-row error-code table (one code, `BRIEF_ARCHIVED`, existed nowhere), `BRIEF_RULES`/`PATCH_RULES` posture, the `module-dependency` record semantics, `spec-review`'s report shapes. A step that depends on a number now points at the response field that carries it. A host test (`lvs-no-contract-copies`) imports the live constants and reads the error-code catalogue off the MCP sources, so a re-created copy fails against the source. `RATIONALE.md` holds the reasoning, unshipped.
...
- **The brief frame states its own filesystem posture, and borrows no block to do it.** It used to claim a ban that nothing enforced (`disallowedTools` is set nowhere in production code); the correction over-shot and gave the frame `<agent_path_scope>`, which the specification places in chat / patch / ask only. Under the shipped `disableDirectFilesystemAccess` default that block handed a brief turn a write permission for directories its frame grants nothing in, and forbade it built-ins absent from its tool catalog — with no `<agent_filesystem_access>` beside it to qualify either, since the brief frame deliberately omits that one too. Both omissions hold now, and the posture is carried by `<interaction_context type="brief">` alone: the brief is read and saved through `get_brief` / `update_brief`, the artifact dirs are write-denied at the sandbox level whatever tools the thread holds, and what tools it holds is left to `<builtin>`, which is derived from that same flag.
