...
### Changed
- **BREAKING: the `database_table` / `ui_view` underscore spellings no longer resolve.** The CLI and the `c4s-reader` MCP accepted `database_table` as an alias for `database-table`. An entity type id is always kebab-case, so that spelling is malformed rather than alternative — and no other type received the courtesy. Use the kebab-case id; `c4s catalog` lists what a project has.
